...
logo_path = os.path.join("DATA", "logo hupa.png") # Asegúrate que el nombre coincida
try:
...
if 'razon_social' not in st.session_state: st.session_state.razon_social = None
    ...
    if st.session_state.loading_stage == 'none':
        ...
        with col:
            ...
            with st.form("login_form"):
                st.markdown("<h3 style='text-align: center; color: white;'>Portal de Razón Social</h3>", unsafe_allow_html=True)
                
                # Definición de usuarios vinculados a su Razón Social
                dict_usuarios = {
                    "GRUPO EMPRESARIAL RRL": "RRL2026",
                    "AGROAVICOLA CHI-HEN": "CHIHEN2026",
                    "AGROPECUARIA NUEVA DEL ORIENTE": "AGNO2026"
                }
                
                u_input = st.selectbox("Seleccione su Empresa", list(dict_usuarios.keys()))
                p_input = st.text_input("Contraseña", type="password")
                submit = st.form_submit_button("INCUBAR SESIÓN", use_container_width=True)
                
                if submit:
                    if p_input == dict_usuarios[u_input]:
                        st.session_state.user = u_input
                        st.session_state.razon_social = u_input
                        st.session_state.loading_stage = 'rocking'
                        st.rerun()
                    else:
                        st.error("Contraseña incorrecta para esta Razón Social")

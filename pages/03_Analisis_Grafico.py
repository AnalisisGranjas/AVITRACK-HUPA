...
if not df_empresa.empty:
    ...
    with cf1:
        # Agregamos "Personalizado" al final de la lista de opciones
        filtro_rapido = st.radio(
            "🔎 Rango de visualización:", 
            ["Últimos 7 días", "Últimos 15 días", "Últimos 30 días", "Ver Todo", "Personalizado"], 
            horizontal=True
        )

    with cf2:
        if filtro_rapido == "Últimos 7 días":
            rango = (max(hoy_ref - datetime.timedelta(days=7), min_f), hoy_ref)
        elif filtro_rapido == "Últimos 15 días":
            rango = (max(hoy_ref - datetime.timedelta(days=15), min_f), hoy_ref)
        elif filtro_rapido == "Últimos 30 días":
            rango = (max(hoy_ref - datetime.timedelta(days=30), min_f), hoy_ref)
        elif filtro_rapido == "Ver Todo":
            rango = (min_f, hoy_ref)
    
        # --- NUEVA LÓGICA: RANGO PERSONALIZADO ---
        elif filtro_rapido == "Personalizado":
            # Desplegamos el calendario de Streamlit para seleccionar Fecha Inicio y Fin
            fechas_sel = st.date_input(
                "Selecciona el rango de fechas:",
                value=(min_f, hoy_ref),  # Valores iniciales por defecto
                min_value=min_f,          # Límite mínimo (primer dato del lote)
                max_value=hoy_ref         # Límite máximo (Ayer)
            )
        
            # Validamos que el usuario haya seleccionado ambas fechas (Inicio y Fin)
            if isinstance(fechas_sel, tuple) and len(fechas_sel) == 2:
                rango = (fechas_sel[0], fechas_sel[1])
            else:
                # Mientras el usuario hace clic en el segundo día, mantiene el rango completo por seguridad
                rango = (min_f, hoy_ref)

        # El st.info actual se mantiene idéntico y mostrará las fechas del calendario automáticamente
    ...
    def render_4_charts(df_input, title_suffix, is_compare=False):
        ...
        with row2_c2:
            with st.container(border=True):
                st.markdown(f"#### 📉 Conversión Gr/Huevo {title_suffix}")
                fig = go.Figure()
                if is_compare:
                    for g in lista_galpones:
                        d = df_input[df_input['Galpón'] == g].sort_values('Fecha')
